...
class Hermes(ShopBenchBaseModel):
    def __init__(self):
        random.seed(AICROWD_RUN_SEED)

        model_path = 'NousResearch/Hermes-2-Theta-Llama-3-8B'
        
        self.tokenizer = AutoTokenizer.from_pretrained('./models/Hermes-2-Theta-Llama-3-8B/', trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained('./models/Hermes-2-Theta-Llama-3-8B/', device_map='auto', torch_dtype=torch.float16, trust_remote_code=True, do_sample=True)
        # self.model = AutoGPTQForCausalLM.from_pretrained('./models/Nous-Hermes-2-Yi-34B/', quantize_config=None, device_map='auto', torch_dtype=torch.float16, trust_remote_code=True, do_sample=True).to('cuda')
