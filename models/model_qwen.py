...
class Qwen(ShopBenchBaseModel):
    def __init__(self):
        random.seed(AICROWD_RUN_SEED)
        
        model_path = 'WDong/Qwen1.5-7B-sft-0502'
        # model_path = 'KnutJaegersberg/Qwen-14B-Llamafied'
        # model_path = 'Qwen/Qwen1.5-MoE-A2.7B-Chat'
        # model_path = 'Qwen/Qwen1.5-MoE-A2.7B-Chat-GPTQ-Int4'
        
        self.tokenizer = AutoTokenizer.from_pretrained('./models/Qwen1.5-7B-sft-0502/', trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained('./models/Qwen1.5-7B-sft-0502/', device_map='auto', torch_dtype=torch.float16, trust_remote_code=True, do_sample=True)
        # self.model = AutoGPTQForCausalLM.from_pretrained('./models/Qwen1.5-MoE-A2.7B-Chat-GPTQ-Int4/', quantize_config=None, device_map='auto', torch_dtype=torch.float16, trust_remote_code=True, do_sample=True).to('cuda')
