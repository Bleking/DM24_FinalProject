...
class DareTies(ShopBenchBaseModel):
    def __init__(self):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained('./models/DARE_TIES_13B/', trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained('./models/DARE_TIES_13B/', device_map='auto', torch_dtype=torch.float16, trust_remote_code=True, do_sample=True)
        # self.model = AutoGPTQForCausalLM.from_pretrained('./models/DARE_TIES_13B/', quantize_config=None, device_map='auto', torch_dtype='auto', trust_remote_code=True, do_sample=True)
    ...
    def predict(self, prompt: str, is_multiple_choice: bool) -> str:
        ...
        if is_multiple_choice:
            if "Which of the following product may have the attribute" or "Select the category of products that can have the attribute" in prompt:  #task 2
